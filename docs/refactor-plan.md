...
| P5 前端 | ✅ 完成 | API 层 + 类型 + react-query；`AppDetail.tsx` 939 行 → 约 100 行装配层，逻辑下沉到 features；路由守卫、错误边界、按角色菜单、分享门户 |
| P6 运维 | ✅ 完成 | Dockerfile/compose/nginx/健康检查/在线备份/CI |
| P7 收尾 | ✅ 完成 | 前端验收（tsc -b / lint / npm ci / build）、前后端联调、文档归位 |
...
4. **数据库运行时验证范围**：**PostgreSQL 18 已实测通过**（迁移建表/索引、应用与通道、上传秒传、发布、v1/v2 检测更新、签名下载字节一致、分享密码、审计，全部正常，含中文 UTF-8 往返）。**MySQL 未做运行时验证**（本机无 Docker/MySQL 服务），代码与迁移 SQL 均使用三方言通用写法，建议在 CI 或预发环境补一次集成验证。
5. **前端类型检查必须用 `tsc -b`**：根 `tsconfig.json` 是 solution 风格（`files: []` + references），原来的 `npx tsc` 实际什么都没检查。已把 build/typecheck 脚本改为 `tsc -b`，并修掉因此暴露的两处真实类型错误。
6. **唯一索引修正（重要）**：早期把可空的 `versions.deleted_at` 放进唯一索引，而 SQLite/PostgreSQL/MySQL 都认为 NULL 互不相等，导致「同一 (应用/平台/通道/版本) 只能有一条活跃记录」实际上**没有被约束**（实测在 PostgreSQL 中确实产生了重复行）。已改为非空的 `deleted_flag`（活跃行 0、软删除行置为自身 id），由迁移 `0005 rebuild_version_unique_index` 负责回填并重建索引，并在 PostgreSQL 上实测确认重复插入被数据库拒绝、软删除后可重新发布。同时修正了迁移框架的一个缺陷：**表结构同步（表/列）现在每次启动都会执行**，否则模型新增的列永远不会加到已迁移过的库上。
